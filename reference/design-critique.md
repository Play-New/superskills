...
- **Feeling alignment:** read the target feeling from CLAUDE.md. Does this touchpoint's first impression match the target feeling? This applies to screens (first 500ms before reading), agent responses (the first line), notifications (the subject line), CLI output (the first line). A touchpoint that is functionally correct but emotionally wrong has failed.
